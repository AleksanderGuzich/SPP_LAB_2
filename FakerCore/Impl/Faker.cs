...
namespace FakerLib.Impl
{
    public class Faker : IFaker
    {
        ...
        private static AbstractGenerator LoadPlugin(string path, Type generatorType)
        {
            ...
            {
                if(IsValidClasses(type)) continue;
                var instance = (AbstractGenerator)Activator.CreateInstance(type);
                if (instance != null && instance.GetType() == generatorType)
                {
                    return instance;
                }
            }
        ...
        private static AbstractGenerator GetGenerator(Type type, Type generatorType)
        {
            var instance = (AbstractGenerator) Activator.CreateInstance(type);
            if (instance != null && instance.GetType() == generatorType)
            {
        ...
        private static bool IsValidClasses(Type type)
        {
            return type == typeof(AbstractGenerator) || type == typeof(AbstractListGenerator) ||
                   type == typeof(GeneratorException) || type == typeof(ListGenerator);
        }
